...
from huffTree import HuffTree, HuffNode
...
class Huffman:
    ...
    def build_huff_map(self, file_str):
        ...
        for i in range(len(file_str)):
            char = file_str[i]
            if self.huff_map.contains_char(char):
                he = self.huff_map.get_huff_elem(char)
                he.inc_freq()
            else:
    ...
    def build_huff_tree(self):
        ...
        2. Create the set of values (HuffElements) from the HuffMap
        3. Build a forest of HuffTrees one from each 
           HuffElement in the HuffMap and enqueue each 
        ...
        for he in self.huff_map.get_value_set():
            tree = HuffTree(element=he)
            huff_pq.enqueue(tree)

        while len(huff_pq) > 1:
            tree1 = huff_pq.dequeue()
            tree2 = huff_pq.dequeue()
            huff_pq.enqueue(HuffTree(left_tree=tree1, right_tree=tree2))
    ...
    def assign_code(self, root):
        ...
        if root.left is not None:
            root.left.set_code(code + "0")
            self.assign_code(root.left)

            root.right.set_code(code + "1")
    ...
    def build_binary_str(self, file_str):
        ...
        binary_str = ""
        for ch in file_str:
            he = self.huff_map.get_huff_elem(ch)
            code = he.get_code()
            binary_str += code
	...
	2. Loop through each character (0 or 1) in the binary_str:
           ...
           b. When you find a leaf node (both left and right node
              ...
              from the HuffNode: 
              i.  Add character to decompressed string 
              ii. Reset the current node pointer to root
        ...
        for i in range(len(binary_str)):
            ...
            if current.left is None and current.right is None:
                decompressed_str += current.get_char()
                current = root
