...
class Vigenere:    
    ...
    def encrypt(self, msg):
        ...
        for ch in msg:
            row = self.get_row_index(self.key[key_index])
            col = self.get_col_index(ch)
            coded_msg = coded_msg + self.vig_squ[row][col]
    ...
    def decrypt(self, coded_msg):
        ...
        key_index = 0
        for ch in coded_msg:
            msg_char = self.get_plain_text_char(ch, self.key[key_index])
    ...
    def get_col_index(self, char):
        ...
        col_index = self.vig_squ[0].index(char)
        return col_index
    ...
    def get_row_index(self, key_char):
        ...
        for row in range(128):
            if self.vig_squ[row][0] == key_char:
                row_index = row
        return row_index
    ...
    def get_plain_text_char(self, coded_char, key_char):
        ...
        col_index = self.vig_squ[row_index].index(coded_char)
        plain_text_char = self.vig_squ[0][col_index]
